...
description: Diagnostic gate that checks AGENTS.md and AGENTS.md for completeness, consistency, and stale artifacts — run right after /cold-start and before /add-feature. Read-only; produces a findings report.
...
1. `AGENTS.md` (project root) — full read
...
| C10 | No Codex-specific directives duplicated wholesale from AGENTS.md | ⚠️ |

### Section B — AGENTS.md structure
| ID | Check | Severity |
|----|-------|----------|
| A1 | Contains no Codex-specific directives (`@import`, memory syntax, hook syntax) | ⚠️ |
...
| ID | Check | Severity |
|----|-------|----------|
| X1 | Build command in AGENTS.md matches build command in AGENTS.md | ❌ |
| X2 | Test command in AGENTS.md matches test command in AGENTS.md | ❌ |
...
| X5 | No rule in AGENTS.md directly contradicts a rule in AGENTS.md | ❌ |
...
- Does not auto-edit AGENTS.md or AGENTS.md.
