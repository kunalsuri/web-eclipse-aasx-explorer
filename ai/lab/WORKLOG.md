...
| W-023 | 2026-07-16 | bugfix | Gate all 39 `server/aasx-routes.ts` route registrations behind `validateAccessToken` (import-only, matching `server/profile.ts`'s existing precedent); closes the gap `W-020` scoped. Adds `tests/integration/aasx-routes-auth.test.ts` (T1 unauthenticated -> `401 MISSING_TOKEN` with no file/metadata side effects, T2 malformed token -> `401 INVALID_TOKEN`, T3 valid token -> unchanged pre-existing behavior, across `GET /files`, `POST /new`, `PUT /environment/:id`, `PATCH /environment/:id/property`, `DELETE /files/:id`) | `ai/lab/specs/SPEC_aasx_routes_auth_middleware.md` | — | — | — | uncommitted (working tree) | `ai/guide/FEATURE_MAP.md`, `ai/analysis/FEATURE_CATALOG.md`, `ai/analysis/audit-reports/DEFECT_TRACEABILITY.md` | in-review | [inferred] |
