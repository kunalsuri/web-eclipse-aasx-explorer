...
| `shared/aas-validation-engine.ts` + `shared/validation-rules/` | AAS V3 constraint validation engine with 117 registered AASd-* IDs (non-contiguous, matching real IDTA gaps), all with real behavioral logic. 33 fabricated non-IDTA IDs (AASd-031..044, AASd-078..089, AASd-091..097) were removed 2026-07-15 (ADV-2026-07-14-03) — they did not correspond to any real IDTA constraint, confirmed against the aas-core-works reference metamodel. Per-file totals overlap; use `tests/unit/shared/validation/aasd/constraint-count.test.ts` only for registration/count checks | `shared/aas-validation-engine.ts` | stable | [inferred] |
